...
const _isValid = (file) => {
  const mimedb = clientVars.md_s3.mimeTypes;
  const mimeType = mimedb[file.type];
  let validMime = null;
  const errorTitle = html10n.get('md_s3.error.title');

  if (clientVars.md_s3 && clientVars.md_s3.fileTypes) {
    validMime = false;
    if (mimeType && mimeType.extensions) {
      for (const fileType of clientVars.md_s3.fileTypes) {
    ...
    if (validMime === false) {
      const errorMessage = html10n.get('md_s3.error.fileType');
  ...
  if (clientVars.md_s3 && file.size > clientVars.md_s3.maxFileSize) {
    const allowedSize = (clientVars.md_s3.maxFileSize / 1000000);
    const errorText = html10n.get('md_s3.error.fileSize', {maxallowed: allowedSize});
...
exports.postToolbarInit = (hook, context) => {
  ...
  toolbar.registerCommand('imageUpload', () => {
    ...
    $(document).find('body').find('#imageInput').on('change', (e) => {
      ...
      if (clientVars.md_s3.storageType === 'base64') {
        ...
        $.ajax({
          type: 'POST',
          url: `${clientVars.padId}/pluginfw/md_s3/upload`,
          xhr: () => {
          ...
          error: (error) => {
            ...
            try {
              ...
              if (errorResponse.type) {
                errorResponse.message = `md_s3.error.${errorResponse.type}`;
              }
            } catch (err) {
